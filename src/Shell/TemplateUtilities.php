...
/**
 * Contains logic for in the Template
 */
class TemplateUtilities
{
    ...
    /**
     * Validate a given URL could be a Wikipedia page
     *
     * @param $sUrl
     *
     * @return bool
     */
    public function isValid($sUrl)
    {
     ...
     * @return string
     */
    public function buildContent($sUrl, Wall $oWall)
    {
        $sContent = '';
        ...
        if ($sUrl !== '') {
            ...
            foreach ($aWordCount as $t_sLanguage => $t_iCount) {
                $sContent .= sprintf(
                    $sTemplate,
                    array_key_exists($t_sLanguage, $this->aLanguages)
                    ?$this->aLanguages[$t_sLanguage]
                    : '--' . $t_sLanguage . '--',
                    $t_iCount
                );
            }
