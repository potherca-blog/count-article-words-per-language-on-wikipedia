<?php
// @FIXME: This file has turned into a mess and *really* needs to be cleaned up!
...
if (PHP_SAPI === "cli") {
    ...
    $sContent = '<!DOCTYPE html><html><head><title>Wikipedia Article Word Counter</title><link rel="stylesheet" href="//cdn.jsdelivr.net/foundation/5.2.2/css/foundation.min.css" /><style>body {padding:1em;}</style></head>';
}
...
if ($sUrl !== '') {
    ...
    if ($bCommandline === true) {
        $sTemplate = '%s'.
            'This script takes one parameter: the URL of the Wikipedia page you'.
            'would like to see this information about.'.
            '%s';
    } else {
        $sTemplate = <<<HTML
            <form
                    ...
                    spellcheck="false"
            >
                <p class="panel callout radius">%s</p>
                <input
                    name="url"
                    type="text"
                    size="32"
...
HTML;
    ...
    $sContent .= sprintf(
        ...
        'Fetching the result could take some time. Please be patient.'
    );
}
...
die($sContent . sprintf("\n\n".'Execution Time: %s seconds', microtime(true) - $fStart));
